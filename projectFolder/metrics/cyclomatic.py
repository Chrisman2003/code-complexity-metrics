...
"""
How can bar “call” foo in the CFG, given that foo’s nodes are completely separate and not in the scope of bar’s labeled nodes?
The answer lies in what the CFG represents:
1) A function call is atomic in the caller <CFG
In bar, the call to foo is just a single node—Clang does not embed foo’s CFG inside bar.
Look at your Clang output:
[B3]
  case 2:
   1: foo
   2: ...
...
B3 represents “call foo and return its result”.
The CFG does not care about the internal nodes of foo; it only knows that after executing B3, control goes to B0 (the next step in bar).
2) The CFG is per function
Each function has a local node numbering (B0, B1, …).
Nodes from foo are completely invisible to bar.
Clang's analyzer is only representing the flow of control within a single function.

3) So how can bar “call” foo?
Objectively:
It doesn't call foo's nodes.
It creates a single CFG node that abstracts the call.
That node accounts for:
Executing foo (without stepping inside)
Returning a value
Passing control to the next statement in bar
Essentially, the call is opaque from the CFG point of view.

✅ Key insight
bar cannot “see” foo's nodes.
The call node in bar represents the entire execution of foo as one atomic step.
If you want a fully inlined/global CFG, you’d have to manually merge function CFGs and adjust edges—but Clang does not do this by default.
If you want, I can illustrate this with a diagram showing bar calling foo as a single node and why it’s independent of foo’s node labels. It usually clears up this confusion instantly.

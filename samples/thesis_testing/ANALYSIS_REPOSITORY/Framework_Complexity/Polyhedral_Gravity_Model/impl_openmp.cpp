...
public:
    GravityEvaluable(
        ...
        : GravityEvaluableBase(Vertices, Faces, density),
          ...
          _segmentVectors(allocateOpenMp<Array3Triplet>(_faces.size(), device_num)),
          _segmentNormals(allocateOpenMp<Array3Triplet>(_faces.size(), device_num)),
          _resultsDevice(allocateOpenMp<GravityModelResult>(_faces.size(), device_num))
    // ,_results(std::make_unique<GravityModelResult[]>(_faces.size()))
    {
    ...
    ~GravityEvaluable() override {
        ...
        omp_target_free(_segmentVectors, device_num);
        omp_target_free(_segmentNormals, device_num);
        omp_target_free(_resultsDevice, device_num);
    }
...
// int dst_device_num = omp_get_initial_device();
// int src_device_num = 0;
// omp_target_memcpy(_results.get(), _resultsDevice, sizeof(GravityModelResult) * _faces.size(), 0, 0, dst_device_num, src_device_num);
#pragma omp target parallel for reduction(+ : result)
        for (size_t i = 0; i < face_count; ++i) {
            result += _resultsDevice[i];
        }
...
#pragma omp target teams distribute parallel for device(device_num)
    ...
    // std::unique_ptr<GravityModelResult[]> _results;
};

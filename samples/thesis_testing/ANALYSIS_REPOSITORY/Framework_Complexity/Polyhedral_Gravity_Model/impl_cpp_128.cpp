...
FloatType neumaier_summation(const std::vector<GravityModelResult> &results) {
    FloatType sum = 0.0;
    FloatType c = 0.0;
    ...
    for (auto r: results) {
        ...
        switch (field) {
            case 0:
                ...
                break;
            case 1:
                num = r.acceleration[0];
                break;
            case 2:
                num = r.acceleration[1];
                break;
            case 3:
                num = r.acceleration[2];
                break;
            default:
                num = r.gradiometricTensor[field - 4];
                break;
        }

        FloatType t = sum + num;
        if (__builtin_fabsf128(sum) >= __builtin_fabsf128(num)) {
            c += (sum - t) + num;
        } else {
            c += (num - t) + sum;
        }
...
#pragma omp parallel for
        ...
        potential = neumaier_summation<0>(_results);
        acceleration[0] = neumaier_summation<1>(_results);
        acceleration[1] = neumaier_summation<2>(_results);
        acceleration[2] = neumaier_summation<3>(_results);

        gradiometricTensor.data[0] = neumaier_summation<4>(_results);
        gradiometricTensor.data[1] = neumaier_summation<5>(_results);
        gradiometricTensor.data[2] = neumaier_summation<6>(_results);
        gradiometricTensor.data[3] = neumaier_summation<7>(_results);
        gradiometricTensor.data[4] = neumaier_summation<8>(_results);
        gradiometricTensor.data[5] = neumaier_summation<9>(_results);

        // std::cout << "P:" << potential << "\n";
...
private:
    void evaluateFace(size_t i, const Array3 &Point) {
        Array3Triplet face = {
                _vertices[_faces[i][0]] - Point,
                _vertices[_faces[i][1]] - Point,
                _vertices[_faces[i][2]] - Point};
        ...
        HessianPlane hessianPlane{};
        {
            ...
            const auto res = (origin - face[0]) * crossProduct;
        ...
        auto planeDistance = __builtin_fabsf128(hessianPlane.d / __builtin_sqrt(
        ...
        Array3 orthogonalProjectionPointOnPlane = _normals[i] * planeDistance;
        {
            ...
            for (unsigned int index = 0; index < 3; ++index) {
                if (intersections[index] < 0) {
                    ...
                    orthogonalProjectionPointOnPlane[index] = __builtin_fabsf128(orthogonalProjectionPointOnPlane[index]);
                } else {
        ...
        for (unsigned int index = 0; index < 3; ++index) {
            auto inner = dot(_segmentNormals[i][index], orthogonalProjectionPointOnPlane - face[index]);
        ...
        for (unsigned int index = 0; index < 3; ++index) {
            if (segmentNormalOrientations[index] == 0) {
                ...
                Matrix columnMatrix = transpose(Matrix{matrixRow1, matrixRow2, matrixRow3});
        ...
        */
            ...
            if (__builtin_fabsf128(distances[index].s1 - distances[index].l1) < EPSILON_ZERO_OFFSET &&
                __builtin_fabsf128(distances[index].s2 - distances[index].l2) < EPSILON_ZERO_OFFSET) {
                ...
                } else if (__builtin_fabsf128(distances[index].s2 - distances[index].s1) < EPSILON_ZERO_OFFSET) {
        ...
        for (unsigned int index = 0; index < 3; ++index) {
            ...
            if ((segmentNormalOrientations[index] == 0 && (r1Norm < EPSILON_ZERO_OFFSET || r2Norm < EPSILON_ZERO_OFFSET)) ||
                (__builtin_fabsf128(distances[index].s1 + distances[index].s2) < EPSILON_ZERO_OFFSET &&
                 __builtin_fabsf128(distances[index].l1 + distances[index].l2) < EPSILON_ZERO_OFFSET)) {
                ...
                if (inner_num <= 0.0 || inner_denom <= 0.0) {// TODO: figure out why, to avoid -inf and -nan
                    ...
                    transcendentalExpressions[index].ln = __builtin_log(inner_num / inner_denom);
                }
            }
            ...
            if (planeDistance < EPSILON_ZERO_OFFSET || segmentDistances[index] < EPSILON_ZERO_OFFSET) {
                ...
                //Implementation of:
                // __builtin_atanhf128(h_p * s2_pq / h_pq * l2_pq) - __builtin_atanhf128(h_p * s1_pq / h_pq * l1_pq)
        ...
        do {
            ...
            if (allInside) {
                singularities = std::make_pair(
                        -1.0 * PI2 * planeDistance,                          //sing alpha = -2pi*h_p
                        _normals[i] * (-1.0 * PI2 * planeNormalOrientation));//sing beta  = -2pi*sigma_p*N_p
                break;
            }
            ...
            if (anyOnLine) {
                singularities = std::make_pair(-1.0 * PI * planeDistance,//sing alpha = -pi*h_p
                                               _normals[i] *
                                                       (-1.0 * PI * planeNormalOrientation));//sing beta  = -pi*sigma_p*N_p
                break;
            }
            ...
            for (unsigned int index = 0; index < 3; ++index) {
                ...
                const FloatType theta = gdot == 0.0 ? PI_2 : __builtin_acoshf128(gdot / (euclideanNorm(g1) * euclideanNorm(g2)));

                singularities = std::make_pair(-1.0 * theta * planeDistance,//sing alpha = -theta*h_p
                                               _normals[i] *
            ...
            if (!anyAtVertex) {
                ...
                singularities = std::make_pair(0.0, Array3{0.0, 0.0, 0.0});
            }
        } while (false);
        ...
        const FloatType planeSumPotentialAcceleration = sum1PotentialAcceleration + planeDistance * sum2 + singularities.first;

        // 6. Step: Sum for tensor
        // consisting of: sum1 + sigma_p * N_p * sum2 + sing B
        // --> Equation 13 the total sum of the brackets
        const Array3 subSum = (sum1Tensor + (_normals[i] * (planeNormalOrientation * sum2))) + singularities.second;
